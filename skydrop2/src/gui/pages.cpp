...
void gui_pages_init()
{
	...
	eeprom_busy_wait();
	if (eeprom_read_byte(&config_ro.factory_passed) !=  CFG_FACTORY_PASSED_hex && only_once)
	{
		only_once = false;
		gui_switch_task(GUI_FACTORY_TEST);
		return;
	}
	...
	if (time_need_set())
	{
		...
		gui_dialog_set_P(PSTR("Clock"), PSTR("Time/Date not set.\nSet time manually\nor wait for GNSS."), GUI_STYLE_TIMESET, gui_pages_set_time_cb);
