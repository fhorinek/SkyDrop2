...
#include "../../fc/conf.h"
#include "../../drivers/sensors/gnss_ublox_m8.h"
...
void gui_set_gps_detail_loop()
{
	...
	if (abs(fc.gps_data.hacc) < 9999 && abs(fc.gps_data.vacc) < 9999)
	{
		disp.GotoXY(0, f_h * 2);
		fprintf_P(lcd_out, PSTR("HACC: %5.2f     VACC: %5.2f"), fc.gps_data.hacc, fc.gps_data.vacc);
	}

	disp.GotoXY(0, f_h * 3);
	fprintf_P(lcd_out, PSTR("Alt: %6.1f"), fc.gps_data.altitude);
...
//			fprintf_P(lcd_out, PSTR("%03d: %d"), fc.gps_data.sat_id[i], fc.gps_data.sat_snr[i]);
		...
		uint8_t x = 10 + 2 * i;
		uint8_t y = GUI_DISP_HEIGHT - 2 - fc.gps_data.sat_id[i] / 6;
		disp.DrawRectangle(x, y, x + 2, GUI_DISP_HEIGHT - 1, 1, 1);
	}
