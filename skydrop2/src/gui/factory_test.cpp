...
void gui_factory_test_loop()
{
	...
	else
	{
		...
		static bool lsm_self_test = false;
...
//			assert(res);
		...
		if (res || blik)
		{
			...
			fprintf_P(lcd_out, PSTR("LSM:%s"), (res) ? "OK" : "ERR");
//			assert(res);
		}
...
//		assert(res);
		...
		{
			fprintf_P(lcd_out, PSTR("BT:4501"));
		}
		disp.GotoXY(8, f_h * 6 + 3);
		fprintf_P(lcd_out, PSTR("t:+%d"), bt_bsel_tune);
	}
...
//		assert(res);
	...
	if (res || blik)
	{
		...
		fprintf_P(lcd_out, PSTR("GNSS:%s"), (res) ? "OK" : "ERR");
//		assert(res);
	}
