...
void fc_init()
{
	...
	fc_meas_timer.EnableInterrupts(timer_overflow | timer_compareA | timer_compareB);
	...
	fc_meas_timer.SetCompare(timer_B, 156); // == 4.992 ms
...
//	DEBUG_TIMER_PWR_ON;
//	Timer test_timer;
//	test_timer.Init(DEBUG_TIMER, timer_div1024);
//	test_timer.SetMode(timer_pwm);
//	test_timer.SetTop(6250);
//	test_timer.SetCompare(timer_compareB, 3125);
//	test_timer.EnableOutputs(timer_B);
//	test_timer.Start();
...
int16_t xtal_calib_stable = 0;

#define XTAL_CALIB_CNT	5

bool fc_xtal_calib_stable()
{
	return xtal_calib_stable > XTAL_CALIB_CNT;
}
...
ISR(FC_MEAS_TIMER_OVF)
{
	...
	if (calib_cnt == 10)
	{
		...
		if (rtc > calib_rtc_cnt)
		{
			...
			{
				if (xtal_calib_stable < XTAL_CALIB_CNT * 2)
					xtal_calib_stable++;
			}
		}
		calib_rtc_cnt = rtc;
...
// * Init lsm6ds i2c readout for FIFO (block the i2c bus)
// * Setup the Buzzer
...
// * Read LSM6DS FIFO (free the I2C bus)
// * Calculate IMU
ISR(FC_MEAS_TIMER_CMPB)
{
	lsm6ds.ReadFIFOStreamAvg(&fc.gyro.raw, &fc.acc.raw);
