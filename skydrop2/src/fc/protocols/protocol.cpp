...
void protocol_step()
{
	if (config.connectivity.forward_gps)
	{
		...
		if (protocol_next_gnss < task_get_ms_tick())
		{
			protocol_next_gnss = task_get_ms_tick() + 1000;

			if (gnss_rmc_msg(msg, sizeof(msg)))
				bt_send(msg);

			if (gnss_gga_msg(msg, sizeof(msg)))
				bt_send(msg);
		}
	}
